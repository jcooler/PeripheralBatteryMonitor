...
A Stream Deck plugin that displays battery status for an explicit ordered list of peripherals. Pressing the key advances to the next selected device; the first selected row is the initial device after the plugin or action starts.
...
4. Use the arrow buttons to set cycle order. Row 1 is marked **Initial**.
...
- Verify device ordering, missing-device retention, and narrow Property Inspector keyboard navigation on a real Stream Deck installation.
