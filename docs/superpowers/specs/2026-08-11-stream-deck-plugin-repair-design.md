...
1. **Passive multi-provider architecture (selected).** Keep SteelSeries GG only for `GET /devices` inventory and receive-only `/sock` events; use exact Windows Bluetooth, XInput, Logitech G Hub, and supported HID reads where those sources provide honest data. This retains the widest useful coverage without invoking GG refresh/configuration operations.
...
The provider contract separates `discover()` from `readStatus(ref)`. A central catalog owns a TTL cache and a shared in-flight discovery promise per provider. Display polling calls only `readStatus`; full Windows PnP, HID, and XInput discovery is never launched by each action tick. Manual refresh explicitly invalidates discovery. Configured references remain in settings when a device is absent and render an honest unavailable/disconnected state.
...
Windows PnP reads battery-bearing device properties in one cached, read-only discovery snapshot. XInput preserves the API's real `Empty`, `Low`, `Medium`, and `Full` levels rather than mapping them to invented percentages; wired means “no battery,” not 100% charging. The HID provider supports only positively identified DualSense/DualSense Edge input reports, never sends output or feature reports, uses serial identity when available, and does not fall back to another controller when an identity disappears. Logitech resolves a stable fingerprint only when unique and never silently selects a same-name device.
